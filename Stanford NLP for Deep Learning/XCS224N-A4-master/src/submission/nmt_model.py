...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(input_size=embed_size + self.hidden_size,     # concat of embeddings + hidden layer
                               hidden_size=self.hidden_size)
        
        self.h_projection = nn.Linear(in_features=self.hidden_size * 2,    # bi-directional
                                        out_features=self.hidden_size, bias=False)

        self.c_projection = nn.Linear(in_features=self.hidden_size * 2,    # bi-directional
                                        out_features=self.hidden_size, bias=False)

        self.att_projection = nn.Linear(in_features=self.hidden_size * 2,    # bi-directional
                                        out_features=self.hidden_size, bias=False)

        self.combined_output_projection = nn.Linear(in_features=self.hidden_size * 3,   # attention added to bi-directional hidden layers
                                        out_features=self.hidden_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Generate tensor of embeddings
        X = self.model_embeddings.source(source_padded)

        # encode X (after passing through pack_padded_sequence)
        enc_hiddens, (last_hidden,last_cell) = self.encoder(pack_padded_sequence(input=X,lengths=source_lengths,enforce_sorted=False))
        ...
        # compute initial decoder states
        init_decoder_hidden = torch.cat([last_hidden[0],last_hidden[1]],dim=1)    # (2, b, h) -> (b, 2*h)
        init_decoder_cell = torch.cat([last_cell[0],last_cell[1]],dim=1)    # (2, b, h) -> (b, 2*h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, decoder_hidden.unsqueeze(2)).squeeze(2)  # shape: (b, src_len)
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)    # shape: (b, src_len) bmm (b, src_len, 2h) = (b, 2h)

        # Concatenate vectors
        U_t = torch.cat((decoder_hidden,a_t), dim=1)    # shape: (b, 3h)

        # Combined output projection
        V_t = self.combined_output_projection(U_t)   # shape: (b, h)

        # Compute output
        O_t = self.dropout(torch.tanh(V_t))     # apply droput after applying tanh 
